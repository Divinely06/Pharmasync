...
export const api = {
  ...
  createSale: (sale: { items: { medicineId: string; quantity: number }[]; discount: number; paymentMethod: string; amountReceived: number; idempotencyKey: string }) => request<{ id: string; paymentId: string | null; status: string; simulated: boolean; duplicate?: boolean }>("/sales", { method: "POST", body: JSON.stringify(sale) }),
