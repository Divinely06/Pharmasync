...
app.get("/api/state", auth, async (_req, res) => {
  const [users, suppliers, medicines, sales, saleItems, inventoryTransactions, auditLogs] = await Promise.all([
    ...
    pool.query('SELECT s.id, s.cashier_id AS "cashierId", u.full_name AS "cashierName", s.transaction_date AS "transactionDate", s.subtotal, s.discount, s.tax, s.total_amount AS "totalAmount", s.payment_method AS "paymentMethod", s.amount_received AS "amountReceived", s.change_amount AS "changeAmount", s.status FROM sales s JOIN users u ON u.id = s.cashier_id ORDER BY s.transaction_date DESC LIMIT 200'),
